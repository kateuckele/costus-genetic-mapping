...
# manhattan plot for plotting multiple chromosomes using base graphics
manhattan_multi <- function(dataframe, limitchromosomes=NULL,pt.col=c('gray10','gray50'),pt.bg=c('gray10','gray50'),
...
# manhattan plot for plotting single chromosomes using base graphics
manhattan_single <- function(dataframe, limitchromosomes=NULL,pt.col=c('gray10','gray50'),pt.bg=c('gray10','gray50'),
	pt.cex=0.45,pch=21,cex.axis=0.95,gridlines=F,gridlines.col='gray83',gridlines.lty=1,gridlines.lwd=1,ymax=10, ymax.soft=T, annotate=NULL,annotate.cex=0.7,annotate.font=3,
	suggestiveline=10, suggestiveline.col='blue', suggestiveline.lwd=1.5, suggestiveline.lty=1, 
	genomewideline=10, genomewideline.col='red', genomewideline.lwd=1.5, genomewideline.lty=1, 
	highlight=NULL,highlight.col=c('green3','magenta'),highlight.bg=c('green3','magenta'),  ...) {

    # Ensure the data has CHR, BP, and P columns
    d = na.omit(dataframe)
    ...
    d$logp = (d$P)  # Convert P-values to -log10 scale for Manhattan plotting

    # Single chromosome plot
    chr_label = unique(d$CHR)
    d$pos = d$BP  # Position is just the base pair for a single chromosome
    ...
    # Create the plot
    plot(d$pos, d$logp, type="n", xaxt='n', yaxt='s', xlim=c(xmin, xmax), ylim=c(0, ymax), 
         xlab=paste("Position on Chromosome", chr_label), ylab=expression(LOD), las=1, cex.axis=cex.axis)

    # Add ticks and labels for base pairs
    bp_ticks <- pretty(d$pos)
    axis(1, at=bp_ticks, labels=bp_ticks, cex.axis=cex.axis)

    # Add points to the plot
    points(d$pos, d$logp, pch=pch, col=pt.col[1], bg=pt.bg[1], cex=pt.cex, ...)

    # Add genome-wide significance line
    if (is.numeric(genomewideline)) {
        abline(h = genomewideline, col = genomewideline.col, lwd = genomewideline.lwd)
    }
