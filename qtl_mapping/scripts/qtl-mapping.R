...
perform_qtl_scan <- function(data_prob, trait_col, trait_name, f1_covar, perm_file, output_path) {
  ...
  # Save results
  write.table(scan_result, file = paste0(output_path, trait_name, "-F1parent.scanone.hk.tsv"), sep = "\t", quote = FALSE)
  saveRDS(scan_result, file = paste0(output_path, trait_name, "-F1parent.scanone.hk.rds"))
...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~##
## 3. PLOT QTL PEAKS
plot_qtl_peaks <- function(scan_result, cutoff_lod, chromosomes, positions, trait_name, output_path) {
  
  if (length(chromosomes) > 4) {
    nrows = ceiling(length(chromosomes) / 4)
    ncols = 4
    pdf_width = 4 * 4
  } else {
    nrows = 1
  ...
  # Save plot to PDF:
  pdf(paste0(output_path, trait_name, "_qtl_peaks.pdf"), width = pdf_width)
...
save_peak_positions <- function(scan_result, cutoff_lod_0.1, cutoff_lod_0.05, trait_name, output_path) {
  write.table(scan_result[scan_result$lod > cutoff_lod_0.1,], 
              file = paste0(output_path, trait_name, "_scanone-hk_peaks-above-0.1LODcutoff.tsv"), sep = "\t", quote = FALSE)
  
  write.table(scan_result[scan_result$lod > cutoff_lod_0.05,], 
              file = paste0(output_path, trait_name, "_scanone-hk_peaks-above-0.05LODcutoff.tsv"), sep = "\t", quote = FALSE)
}

##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~##
## 5. PLOT QTL EFFECTS
plot_qtl_effects <- function(data_prob, chromosomes, positions, trait_name, output_path) {
  ...
  ## Save plot to PDF:
  pdf(paste0(output_path, trait_name, "_qtl_effect_plot.pdf"), width = pdf_width)
  par(mfrow = c(nrows, ncols))
...
calc_lod_intervals <- function(scan_result, chromosomes, prob_bayes, output_path) {
  ...
  write.table(out,
              file      = paste0(output_path, trait_name, "_peak_intervals.tsv"),
              sep       = "\t",
              quote     = FALSE,
              row.names = FALSE)
...
plot_manhattan <- function(scanone_results, cutoff_lod, trait_name, output_path) {
  ## load the qqman manhattan plot function
  source("./scripts/adapted_qqman.R")
  pdf(paste0(output_path, trait_name, "_manhattan_plot.pdf"), width = 10, height = 6)
  par(mfrow = c(1, 1))
...
perm_file <- "./results/processed_data/scanone_1000perm.rds"
...
for (trait_name in names(data_prob$pheno)[2:50]) {
  ...
  scan_result <- perform_qtl_scan(data_prob, trait_name, trait_name, F1parent, 
                                  perm_file, output_path = "./results/data_frames/")
  ...
  save_peak_positions(scan_result$scan_result, scan_result$cutoff_lod_0.1, 
                      ...
                      output_path = "./results/data_frames/")
  ...
  # Plot QTL effects
  plot_qtl_effects(data_prob = data_prob, chromosomes = qtl_peaks$alpha_0.1$chr, 
