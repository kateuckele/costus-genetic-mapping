...
#bract_spec_scalemax <- procspec(bract_spec_sm, opt = c("max"))
#petal_spec_scalemax <- procspec(petal_spec_sm, opt = c("max"))
#labellum_spec_scalemax <- procspec(labellum_spec_sm, opt = c("max"))

bract_spec_scalemax <- bract_spec_sm
petal_spec_scalemax <- petal_spec_sm
labellum_spec_scalemax <- labellum_spec_sm
```
...
```{r define-params}
# duplicate the spectral data specifically for plotting
bract_spec_plotting <- bract_spec_scalemax
petal_spec_plotting <- petal_spec_scalemax
labellum_spec_plotting <- labellum_spec_scalemax

# Duplicate the parental columns
bract_spec_plotting$`125.2` <- unlist(bract_spec_plotting[which(names(bract_spec_plotting) == "125")])
bract_spec_plotting$`126.2` <- unlist(bract_spec_plotting[which(names(bract_spec_plotting) == "126")])
bract_spec_plotting$BRAC.2 <- unlist(bract_spec_plotting[which(names(bract_spec_plotting) == "BRAC")])

petal_spec_plotting$`125.2` <- unlist(petal_spec_plotting[which(names(petal_spec_plotting) == "125")])
petal_spec_plotting$`126.2` <- unlist(petal_spec_plotting[which(names(petal_spec_plotting) == "126")])
petal_spec_plotting$BRAC.2 <- unlist(petal_spec_plotting[which(names(petal_spec_plotting) == "BRAC")])

labellum_spec_plotting$`125.2` <- unlist(labellum_spec_plotting[which(names(labellum_spec_plotting) == "125")])
labellum_spec_plotting$`126.2` <- unlist(labellum_spec_plotting[which(names(labellum_spec_plotting) == "126")])
labellum_spec_plotting$BRAC.2 <- unlist(labellum_spec_plotting[which(names(labellum_spec_plotting) == "BRAC")])

# Define a color palette for highlighting specific samples
colors_bract <- spec2rgb(bract_spec_plotting)
colors_petal <- spec2rgb(petal_spec_plotting)
colors_labellum <- spec2rgb(labellum_spec_plotting)

# Change the colors of the duplicated parentals
colors_bract[which(names(colors_bract) == "125.2")] <- "black"
colors_bract[which(names(colors_bract) == "126.2")] <- "black"
colors_bract[which(names(colors_bract) == "BRAC.2")] <- "red"

colors_petal[which(names(colors_petal) == "125.2")] <- "black"
colors_petal[which(names(colors_petal) == "126.2")] <- "black"
colors_petal[which(names(colors_petal) == "BRAC.2")] <- "red"

colors_labellum[which(names(colors_labellum) == "125.2")] <- "black"
colors_labellum[which(names(colors_labellum) == "126.2")] <- "black"
colors_labellum[which(names(colors_labellum) == "BRAC.2")] <- "red"
```
...
order_spec_bract <- c(
  which(names(bract_spec_plotting) == "125"),
  which(names(bract_spec_plotting) == "126"),
  which(names(bract_spec_plotting) == "BRAC"),
  which(names(bract_spec_plotting) == "125.2"),
  which(names(bract_spec_plotting) == "126.2"),
  which(names(bract_spec_plotting) == "BRAC.2")
)

order_spec_bract_col <- c(
  which(names(colors_bract) == "125"),
  which(names(colors_bract) == "126"),
  which(names(colors_bract) == "BRAC"),
  which(names(colors_bract) == "125.2"),
  which(names(colors_bract) == "126.2"),
  which(names(colors_bract) == "BRAC.2")
)

# Reorder columns to place highlighted samples at the beginning
columns_bract <- 1:ncol(bract_spec_plotting)
order_spec_bract <- c(order_spec_bract, columns_bract[columns_bract %notin% order_spec_bract])

columns_bract_color <- 1:length(colors_bract)
order_spec_bract_col <- c(order_spec_bract_col, columns_bract_color[columns_bract_color %notin% order_spec_bract_col])

# Plot
plot(bract_spec_plotting[order_spec_bract], 
     type = "o",
     col = colors_bract[order_spec_bract_col], 
     main = "Bract Spectra",
     xlab = "Wavelength (nm)", 
     ylab = "Reflectance (%)")
```
...
# Order samples to highlight specific ones
order_spec_petal <- c(
  which(names(petal_spec_plotting) == "125"),
  which(names(petal_spec_plotting) == "126"),
  which(names(petal_spec_plotting) == "BRAC"),
  which(names(petal_spec_plotting) == "125.2"),
  which(names(petal_spec_plotting) == "126.2"),
  which(names(petal_spec_plotting) == "BRAC.2")
)

order_spec_petal_col <- c(
  which(names(colors_petal) == "125"),
  which(names(colors_petal) == "126"),
  which(names(colors_petal) == "BRAC"),
  which(names(colors_petal) == "125.2"),
  which(names(colors_petal) == "126.2"),
  which(names(colors_petal) == "BRAC.2")
)

# Reorder columns to place highlighted samples at the beginning
columns_petal <- 1:ncol(petal_spec_plotting)
order_spec_petal <- c(order_spec_petal, columns_petal[columns_petal %notin% order_spec_petal])

columns_petal_color <- 1:length(colors_petal)
order_spec_petal_col <- c(order_spec_petal_col, columns_petal_color[columns_petal_color %notin% order_spec_petal_col])

# Plot
plot(petal_spec_plotting[order_spec_petal], 
     type = "o",
     col = colors_petal[order_spec_petal_col], 
     main = "Petal Spectra",
     xlab = "Wavelength (nm)", 
     ylab = "Reflectance (%)")
```
...
order_spec_labellum <- c(
  which(names(labellum_spec_plotting) == "125"),
  which(names(labellum_spec_plotting) == "126"),
  which(names(labellum_spec_plotting) == "BRAC"),
  which(names(labellum_spec_plotting) == "125.2"),
  which(names(labellum_spec_plotting) == "126.2"),
  which(names(labellum_spec_plotting) == "BRAC.2")
)

order_spec_labellum_col <- c(
  which(names(colors_labellum) == "125"),
  which(names(colors_labellum) == "126"),
  which(names(colors_labellum) == "BRAC"),
  which(names(colors_labellum) == "125.2"),
  which(names(colors_labellum) == "126.2"),
  which(names(colors_labellum) == "BRAC.2")
)

# Reorder columns to place highlighted samples at the beginning
columns_labellum <- 1:ncol(labellum_spec_plotting)
order_spec_labellum <- c(order_spec_labellum, columns_labellum[columns_labellum %notin% order_spec_labellum])

columns_labellum_color <- 1:length(colors_labellum)
order_spec_labellum_col <- c(order_spec_labellum_col, columns_labellum_color[columns_labellum_color %notin% order_spec_labellum_col])

# Plot
plot(labellum_spec_plotting[order_spec_labellum], 
     type = "o",
     col = colors_labellum[order_spec_labellum_col], 
     main = "Labellum Spectra",
     xlab = "Wavelength (nm)", 
     ylab = "Reflectance (%)")
```
...
pdf("~/Dropbox/Costus/costus-genetic-mapping/phenotype/results/figures/Combined_Spectra_Bract_Petal_Labellum_Cbracteatus_Clasius_hybrids_alternate.pdf", width = 25, height = 7)
par(mfrow = c(1, 3))
...
spec.bin <- procspec(bract_spec_scalemax, opt = c("bin", "center"))
...
pca_bract <- data.frame(PC1_bract=pca1$x[,1], PC2_bract=pca1$x[,2], PC3_bract=pca1$x[,3])
```
## Principal component analysis - Petal 
```{r}
# opt = 'center' centers the spectra to have a mean reflectance of zero (thus removing brightness as a dominant variable in the PCA)
# opt = 'bin' bins the spectra into user-defined bins
spec.bin <- procspec(petal_spec_scalemax, opt = c("bin", "center"))
...
# Extracting PCA Scores
pca_petal <- data.frame(PC1_petal=pca1$x[,1], PC2_petal=pca1$x[,2], PC3_petal=pca1$x[,3])
```
...
# opt = 'center' centers the spectra to have a mean reflectance of zero (thus removing brightness as a dominant variable in the PCA)
# opt = 'bin' bins the spectra into user-defined bins
spec.bin <- procspec(labellum_spec_scalemax, opt = c("bin", "center"))
...
# Extracting PCA Scores
pca_labellum <- data.frame(PC1_labellum=pca1$x[,1], PC2_labellum=pca1$x[,2], PC3_labellum=pca1$x[,3])
```
## Format PC Scores 
```{r}
...
summary_bract <- summary(bract_spec_scalemax)
...
summary_petal <- summary(petal_spec_scalemax)
...
summary_labellum <- summary(labellum_spec_scalemax)
...
```{r scale UV}
# Scale spectra to both minimum and maximum reflectance
bract_spec_scalemax_UV <- procspec(bract_spec_sm, opt = c("min", "max"))
petal_spec_scalemax_UV <- procspec(petal_spec_sm, opt = c("min", "max"))
labellum_spec_scalemax_UV <- procspec(labellum_spec_sm, opt = c("min", "max"))
```
...
# Order samples to highlight specific ones
order_spec_bract <- c(
  which(names(bract_spec_scalemax_UV) == "125"),
  which(names(bract_spec_scalemax_UV) == "126"),
  which(names(bract_spec_scalemax_UV) == "BRAC")
)

# Reorder columns to place highlighted samples at the end
columns_bract <- 1:ncol(bract_spec_scalemax_UV)
order_spec_bract <- c(columns_bract[columns_bract %notin% order_spec_bract], order_spec_bract)

# Plot the spectra
plot(bract_spec_scalemax_UV[order_spec_bract], type = "o",
...
# Order samples to highlight specific ones
order_spec_petal <- c(
  which(names(petal_spec_scalemax_UV) == "125"),
  which(names(petal_spec_scalemax_UV) == "126"),
  which(names(petal_spec_scalemax_UV) == "BRAC")
)

# Reorder columns to place highlighted samples at the end
columns_petal <- 1:ncol(petal_spec_scalemax_UV)
order_spec_petal <- c(columns_petal[columns_petal %notin% order_spec_petal], order_spec_petal)

# Plot the spectra
plot(petal_spec_scalemax_UV[order_spec_petal], type = "o",
...
# Order samples to highlight specific ones
order_spec_labellum <- c(
  which(names(labellum_spec_scalemax_UV) == "125"),
  which(names(labellum_spec_scalemax_UV) == "126"),
  which(names(labellum_spec_scalemax_UV) == "BRAC")
)

# Reorder columns to place highlighted samples at the end
columns_labellum <- 1:ncol(labellum_spec_scalemax_UV)
order_spec_labellum <- c(columns_labellum[columns_labellum %notin% order_spec_labellum], order_spec_labellum)

# Plot the spectra
plot(labellum_spec_scalemax_UV[order_spec_labellum], type = "o",
...
# Increase left margin to provide more space for y-axis labels
par(mar = c(5, 5, 4, 2) + 0.1)  # c(bottom, left, top, right)
plot(bract_spec_scalemax_UV[order_spec_bract], type = "o", col = highlight_colors, 
...
plot(petal_spec_scalemax_UV[order_spec_petal], type = "o", col = highlight_colors, 
     main = "Petal spectra", xlab = "Wavelength (nm)", ylab = "Reflectance (%)",
     cex.main = 3, cex.lab = 2.5, cex.axis = 2, lwd = 2)
legend("topleft", legend = c("C. bracteatus parent", "C. lasius parent"),
       col = c("darkgreen", "gold1"), lty = 1, cex = 2.5, bty = "n")
plot(labellum_spec_scalemax_UV[order_spec_labellum], type = "o", col = highlight_colors, 
...
```{r bract-summary UV}
# Calculate spectral descriptors for bract
summary_bract_UV <- summary(bract_spec_scalemax_UV)
...
```{r petal-summary UV}
# Calculate summary statistics for petal
summary_petal_UV <- summary(petal_spec_scalemax_UV)
...
# Calculate summary statistics for labellum
summary_labellum_UV <- summary(labellum_spec_scalemax_UV)
...
# Vision Models
```{r}
# Compute Photoreceptor Quantum Catches for Birds and Bees
## Bract
bract_bird_vis <- vismodel(bract_spec_scalemax, visual = "avg.uv", relative = TRUE)
bract_bee_vis <- vismodel(bract_spec_scalemax, visual = "apis", relative = TRUE)

## Petal
petal_bird_vis <- vismodel(petal_spec_scalemax, visual = "avg.uv", relative = TRUE)
petal_bee_vis <- vismodel(petal_spec_scalemax, visual = "apis", relative = TRUE)

## Labellum
labellum_bird_vis <- vismodel(labellum_spec_scalemax, visual = "avg.uv", relative = TRUE)
labellum_bee_vis <- vismodel(labellum_spec_scalemax, visual = "apis", relative = TRUE)

# Convert Bird Vision Data into a Color Space
bract_bird_colspace <- colspace(bract_bird_vis, space = "auto")
petal_bird_colspace <- colspace(petal_bird_vis, space = "auto")
labellum_bird_colspace <- colspace(labellum_bird_vis, space = "auto")

# Plot the Avian Color Space Representation
plot(bract_bird_colspace, col = spec2rgb(bract_spec_scalemax))
projplot(bract_bird_colspace, pch = 20, col = spec2rgb(bract_spec_scalemax))

plot(petal_bird_colspace, col = spec2rgb(petal_spec_scalemax))
projplot(petal_bird_colspace, pch = 20, col = spec2rgb(petal_spec_scalemax))

plot(labellum_bird_colspace, col = spec2rgb(labellum_spec_scalemax))
projplot(labellum_bird_colspace, pch = 20, col = spec2rgb(labellum_spec_scalemax))

# Convert Bee Vision Data into a Color Space
bract_bee_colspace <- colspace(bract_bee_vis, space = "tri")
petal_bee_colspace <- colspace(petal_bee_vis, space = "tri")
labellum_bee_colspace <- colspace(labellum_bee_vis, space = "tri")

# Plot the Bee Color Space Representation
hexplot(bract_bee_colspace, col = spec2rgb(bract_spec_scalemax))
hexplot(petal_bee_colspace, col = spec2rgb(petal_spec_scalemax))
hexplot(labellum_bee_colspace, col = spec2rgb(labellum_spec_scalemax))

# Replace 'x' with '_' in row names
rownames(bract_bird_colspace) <- gsub("x", "_", rownames(bract_bird_colspace))
rownames(bract_bee_colspace) <- gsub("x", "_", rownames(bract_bee_colspace))

rownames(petal_bird_colspace) <- gsub("x", "_", rownames(petal_bird_colspace))
rownames(petal_bee_colspace) <- gsub("x", "_", rownames(petal_bee_colspace))

rownames(labellum_bird_colspace) <- gsub("x", "_", rownames(labellum_bird_colspace))
rownames(labellum_bee_colspace) <- gsub("x", "_", rownames(labellum_bee_colspace))

# Convert row names to a column named 'id'
bract_bird_colspace <- bract_bird_colspace %>% rownames_to_column(var = "id")
bract_bee_colspace <- bract_bee_colspace %>% rownames_to_column(var = "id")

petal_bird_colspace <- petal_bird_colspace %>% rownames_to_column(var = "id")
petal_bee_colspace <- petal_bee_colspace %>% rownames_to_column(var = "id")

labellum_bird_colspace <- labellum_bird_colspace %>% rownames_to_column(var = "id")
labellum_bee_colspace <- labellum_bee_colspace %>% rownames_to_column(var = "id")

# Subset relevant columns based on analysis needs
bract_bird_colspace <- bract_bird_colspace[, c("id", "h.theta", "r.vec")]
bract_bee_colspace <- bract_bee_colspace[, c("id", "h.theta", "r.vec")]

petal_bird_colspace <- petal_bird_colspace[, c("id", "h.theta", "r.vec")]
petal_bee_colspace <- petal_bee_colspace[, c("id", "h.theta", "r.vec")]

labellum_bird_colspace <- labellum_bird_colspace[, c("id", "h.theta", "r.vec")]
labellum_bee_colspace <- labellum_bee_colspace[, c("id", "h.theta", "r.vec")]

# Rename columns to include tissue type
colnames(bract_bird_colspace) <- c("id", "h.theta.bird.bract", "r.vec.bird.bract")
colnames(bract_bee_colspace) <- c("id", "h.theta.bee.bract", "r.vec.bee.bract")

colnames(petal_bird_colspace) <- c("id", "h.theta.bird.petal", "r.vec.bird.petal")
colnames(petal_bee_colspace) <- c("id", "h.theta.bee.petal", "r.vec.bee.petal")

colnames(labellum_bird_colspace) <- c("id", "h.theta.bird.labellum", "r.vec.bird.labellum")
colnames(labellum_bee_colspace) <- c("id", "h.theta.bee.labellum", "r.vec.bee.labellum")
```
...
joined_df <- pca_bract %>%
  ...
  full_join(bract_bird_colspace, by = "id") %>%
  full_join(bract_bee_colspace, by = "id") %>%
  full_join(petal_bird_colspace, by = "id") %>%
  full_join(petal_bee_colspace, by = "id") %>%
  full_join(labellum_bird_colspace, by = "id") %>%
  full_join(labellum_bee_colspace, by = "id")
```
